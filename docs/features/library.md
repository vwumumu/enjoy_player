...
- **Search**: `librarySearchProvider` filters Library lists by title (case-insensitive). The sidebar search field is visible only with the rail; **below the rail breakpoint**, Library shows a **compact search field** under the editorial header so mobile/tablet users can filter. **Empty vs filtered**: when the search query is non-empty and the current tab’s kind has items in the library but none match, the tab shows a **No matches** empty state with **Clear search**; when the library truly has no items of that kind, the tab shows the existing **empty library** copy.
