...
- **Live cover frame** (`capturePracticePosterEchoFrame`): when the active player exposes a screenshot surface (`media_kit` `Player.screenshot`), the cover is captured as a `Uint8List` and bound to `echoCoverBytes`. On YouTube WebView (no native screenshot surface), the resolver falls through to the local / network thumbnail.
