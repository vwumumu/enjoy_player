...
class _MultiYoutubeClient implements YoutubeTranscriptsClient {
  ...
  Future<Map<String, dynamic>> pollTranscripts({
    ...
    if (sequence != null) {
      final idx =
          _seqIndex < sequence!.length ? _seqIndex : sequence!.length - 1;
      _seqIndex++;
...
void main() {
  group('TranscriptRepository bilingual', () {
    ...
    test('T006: native base == source routes to single-language path', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
    ...
    test('T006: differing native uses multi-language path with base codes', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en-US');
    ...
    test('T007/T008: ready(2) upserts two rows and assigns primary/secondary', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
      ...
      final session = await db.echoSessionDao.getLatestForTarget('Video', mediaId);
      expect(session?.transcriptId, enId); // primary = source/original
      expect(session?.secondaryTranscriptId, zhId); // secondary = native
    });

    test('T016a: partial missing translation stores original, success, no error', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
    ...
    test('T016b: partial missing source stores translation, no invented primary', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
    ...
    test('T018: und source language skips cloud (no worker calls)', () async {
      ...
      final client = _MultiYoutubeClient(
        transcripts: {'status': 'ready', 'transcripts': [_cue('en')]},
      );
      final repo = TranscriptRepository(db, null, client);
      final result = await repo.resolveOnOpen(
        mediaId,
    ...
    test('T023: long-poll sends non-zero waitMs and retries until ready', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
      ...
      final client = _MultiYoutubeClient(
        sequence: [
          {'status': 'generating', 'jobId': 'j', 'stage': 'running', 'created': true},
          {'status': 'generating', 'jobId': 'j', 'stage': 'running', 'created': false},
          {
            'status': 'ready',
            'videoId': vid,
            'transcripts': [_cue('en'), _cue('zh')],
    ...
    test('T023: generating exhausts the attempt budget and records error', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
    ...
    test('T024: already-fetched target is skipped with zero worker calls', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
    ...
    test('forceRefresh bypasses the already-fetched skip on the multi path', () async {
      const vid = 'abcdefghijk';
      final mediaId = enjoyVideoId(provider: 'youtube', vid: vid);
      await insertYoutubeVideo(mediaId: mediaId, vid: vid, language: 'en');
